...
```jsx
// Incorrect
import { Listitem } from '@rneuinext/themed';
...
1. Check your `package.json` for the version of `@rneuinext/themed` you are
   using.
...
2. Install React Native Elements
   ...
   ```bash
   # yarn
   yarn && yarn add @rneuinext/base @rneuinext/themed
   ```
